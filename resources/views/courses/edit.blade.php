        ...
        <div class="row justify-content-center">
            <div class="col-lg-5 col-md-8">
                <div class="card card-register shadow-lg p-4">
                    ...
                    <form id="form-edit-course" action="{{ route('courses.update', $courses->id) }}" method="post">                        @csrf
                        @method('PUT')


                        <div class="form-group mt-3">
                            <label for="course_name">Nombre del Curso</label>
                            <input type="text" name="course_name" id="course_name" value="{{ $courses->course_name }}"  class="form-control">
                        </div>


                        <div class="form-group mt-3">
                            <label for="course_description">Descripcion</label>
                            <input type="text" name="course_description" id="course_description" value="{{ $courses->course_description }}"  class="form-control">
                        </div>


                        <div class="form-group mt-3">
                            <label for="course_duration">Duracion (H)</label>
                            <input type="number" name="course_duration" id="course_duration" value="{{ $courses->course_duration }}"  class="form-control">
                        </div>


                        <div class="d-flex justify-content-between mt-4">
                            <button type="submit" class="btn btn-success w-50">Editar</button>
                            <a href="{{ route('courses.index') }}" class="btn btn-primary w-50 ml-2">Atrás</a>
                        </div>
                    </form>
                </div>
            </div>
