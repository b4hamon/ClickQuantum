            ...
            <div class="row justify-content-center">
                <div class="col-lg-5 col-md-8">
                    <div class="card card-register shadow-lg p-4">
                        <form id="form-edit-comment" action="{{ route('comments.update', $comment->id) }}"
                            method="post">
                            @csrf
                            @method('PUT')
                            <div class="form-group mt-3">
                                ...
                                <input type="text" name="comment_content" id="comment_content"
                                    value="{{ $comment->comment_content }}"  class="form-control">
                            </div>

                            <div class="form-group mt-3">
                                <label for="comment_date">Fecha</label>
                                <input type="date" name="comment_date" id="comment_date"
                                    value="{{ old('comment_date', $comment->comment_date ? date('Y-m-d', strtotime($comment->comment_date)) : '') }}"
                                     class="form-control">
                            </div>
                                ...
                                <select name="article_id" id="article_id" class="form-control" >
                                    ...
                                    @foreach ($articles as $article)
                                        <option value="{{ $article->id }}"
                                            {{ old('article_id', $comment->article_id) == $article->id ? 'selected' : '' }}>
                                            {{ $article->title }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="user_id" id="user_id" class="form-control" >
                                    ...
                                    @foreach ($users as $users)
                                        <option value="{{ $users->id }}"
                                            {{ old('user_id', $comment->user_id) == $users->id ? 'selected' : '' }}>
                                            {{ $users->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="d-flex justify-content-between mt-4">
                                ...
                                <a href="{{ route('comments.index') }}" class="btn btn-primary w-50 ml-2">Atrás</a>
                            </div>
                        </form>
                    </div>
                </div>
